            ...
            <Modal title={editing ? "Sửa Sản phẩm" : "Thêm mới Sản phẩm"} open={modalVisible}
                ...
                <Form form={form} layout="vertical">
                    <div style={{ display: 'flex', gap: 16 }}>
                        <Form.Item name="name" label="Tên sản phẩm" rules={[{ required: true, message: 'Bắt buộc!' }]} style={{ flex: 1 }}>
                            <Input onChange={(e) => {
                                if (!editing) {
                                    const slug = e.target.value.toLowerCase().trim().replace(/[\s\W-]+/g, '-');
                                    form.setFieldsValue({ slug });
                        ...
                        <Form.Item name="slug" label="Slug" rules={[{ required: true, message: 'Bắt buộc!' }]} style={{ flex: 1 }}>
                            <Input />
                        </Form.Item>
                    </div>
                    ...
                    <Form.Item name="title" label="Tiêu đề phụ" rules={[{ required: true }]}>
                        <Input />
                    </Form.Item>
